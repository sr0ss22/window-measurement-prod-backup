...
import { FileClock, ArrowLeft, Home, ClipboardList, Calendar } from "lucide-react";
...
export default function PendingAcceptancePage() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    if (error) {
      console.error("Error fetching pending acceptance projects:", error);
      ...
      <main className="relative z-10">
          ...
          <div className="ios:pt-[env(safe-area-inset-top)] h-full">
            <div className="container mx-auto px-4 text-white flex justify-between items-center h-full">
              <div className="flex items-center gap-2">
                ...
                <FileClock className="h-7 w-7 text-primary-foreground" />
                <h1 className="text-lg font-bold">Pending Acceptance</h1>
              </div>
              <UserNav />
            </div>
          </div>
        ...
        <div className="bg-white rounded-t-[32px] shadow-lg border-t-4 border-[#F08200] -mt-8 min-h-[calc(100vh-150px-64px)] md:min-h-[calc(100vh-120px-64px)]">
          <div className="container mx-auto px-4 pt-6 pb-20">
            {!isMobile && <p className="text-lg text-gray-600 mb-6">Projects awaiting customer acceptance.</p>}
            <ProjectList
              projects={projects}
